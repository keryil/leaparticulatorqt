...
sca(axes[1])
[plot_discretized(t, symbol_generator, color=c, alpha=alpha, figure=1, label="%s" % tr.index(t)) for c, t in
 zip(colors * 5, tr[:limit])]
# sca(axes[3])
        ...
        for j in range(len(means)):
            ...
            if i != j:
                ...
                q = quiver([x0], [y0], [x1 - x0], [y1 - y0], alpha=alpha,
                           ...
                           label="P(%d->%d)=%f" % (i, j, transmat[i][j]))
                legend()

        annotations.append(
            annotate(s=prob_string, xy=means_[i], xytext=(0, 10), xycoords="data", textcoords="offset points",
                     alpha=1, bbox=dict(boxstyle='round,pad=0.2', fc=colorConverter.to_rgb(colors[i]), alpha=0.3),
                     picker=True,
                     visible=False))
...
def train_hmm_once(file_id, nstates, iter=1000):
    ...
    rpy2.ipython
    % R
    source("~/Dropbox/ABACUS/Workspace/LeapArticulator/SampleHMM.R")
    % Rpush
    file_id
    % Rpush
    nstates
    % R
    list[hmm, d] = fitHMM(file_id, nstates, iter=1000)
    % Rpull
    d
    % Rpull
    hmm
