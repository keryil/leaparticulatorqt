...
responses, test_results, responses_practice, test_results_practice, images = ExperimentalData.fromFile("logs/1230105514.master.exp.log")
...
formatData = lambda r, phase: [[(palmToAmpAndFreq(frame.get_stabilized_position())[::-1], 
                                 extract_velocity(frame)) for frame in rr] for rr in r["127.0.0.1"][str(phase)].values()]
