...
from leaparticulator.data.hmm import reconstruct_hmm, reduce_hmm
...
def plot_quiver2d(data, alpha=.75, C=[], path=None, *args, **kwargs):
    ...
    patches = quiver(X, Y, U, V, *args, color=C, edgecolors=["black" for i in C], scale_units='xy', angles='xy',
                     scale=1, width=0.005, alpha=alpha, **kwargs)
    return patches
...
def pickle_results(results, nstates, trials, iter, filename_log, phase=None, units=constants.XY):
    hmms, ds = zip(*[reduce_hmm(hmm)[1] for hmm in results])
    assert any(hmms)
    assert any(ds)
    # print_n_flush(hmms)
...
def unpickle_results(filename_log, phase=None, units=None):
    ...
    with open(filename_log + extension, "r") as f:
        ...
        # print ds
        hmms = [reconstruct_hmm(hmm, d) for hmm, d in zip(hmms, ds)]
...
return stationary_dist  # print stationary(results.hmms[1].transmat)
