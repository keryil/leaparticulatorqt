  ...
  draw_model_coefs<- function(model, subset) {
    ...
    p <- p + labs(x= "Phase", y="Coefficient") +  scale_colour_grey() + scale_fill_grey() + theme(
      ...
      panel.grid.minor = element_line(size = 0.25, linetype = 'solid',
                                      colour = "gray")) 
    p <- p + ylim(c(-.1,.2)) #+ theme(axis.title.x = element_blank()) 
    return(p)
    # }
  ...
  image_filename <- '/home/kerem/Dropbox/ABACUS/Publications/journalpapes/images/fixed_effects_discrete_separate.tex'
  ...
  outfile <- paste(paste(str_split(image_filename, "/")[[1]][1:8],collapse="/"), "fixed_effects_discrete_separate.pdf", sep="/")
  file.copy(from="fixed_effects_discrete_separate.pdf", to=outfile, overwrite = TRUE)
  file.remove("fixed_effects_discrete_separate.pdf")
